...
if (!class_exists('clean_comments_constructor')) {
	class clean_comments_constructor extends Walker_Comment {
        ...
        echo "</span>";
	    	...
	    	echo '<span class="meta media-time">'.get_comment_date('d F Y')."\n";
         echo "</span>";
...
if (!function_exists('add_scripts')) {
	function add_scripts() {
	    ...
	    wp_enqueue_script('jquery','//ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js','','',true);
...
function top_views($args) {
	...
	$trans_prefix = 'top_views_' . $args['range'] . '_' . $args['limit'];

	if ( false === ( $results = get_transient($trans_prefix) ) ) {
		$results = $wpdb->get_results($query);
		set_transient($trans_prefix, $results,MINUTE_IN_SECONDS );
	}
